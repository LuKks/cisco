...
module.exports = class Cisco {
  ...
  async receive (message) {
    ...
    if (!this.quiet) {
      ...
      console.log(open + answer.out.content.replace(SEARCH_REPLACE, function replacer (match, filename, language, block, search, replace) {
...
${highlight(replace.trim(), { language })}
${crayon.redBright(crayon.bold('>>>>>>>'))} ${crayon.greenBright(crayon.bold('REPLACE'))}
        `.trim()) + open
      }) + close)

      // console.log(crayon.gray('Tokens: ' + answer.usage.prompt_tokens + ' sent, ' + answer.usage.completion_tokens + ' received.'))
    }

    for (const [, filename, , , search, replace] of changes) {
      ...
      // TODO: Check if the search matches?
...
function getPlatformInfo () {
  ...
  const info = {
    ...
    date: new Date().toLocaleString('en-CA'), // YYYY-MM-DD
