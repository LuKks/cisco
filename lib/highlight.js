...
module.exports = function highlight (code, opts = {}) {
  ...
  try {
    highlighted = hljs.highlight(code, { language: opts.language })
  } catch (err) {
    if (err.message.includes('Unknown language')) {
      return code
    }
  ...
  return htmlToText(highlighted.value, {
    ...
    formatters: {
      highlight: function (elem, walk, builder, formatOptions) {
        ...
        // TODO: Improve coloring for other languages
        const theme = THEMES[opts.language] || THEMES.javascript
        const color = theme ? (theme[type + '-' + content.slice(0, 32)] || theme[type]) : null
        ...
        try {
          if (opts.language === 'javascript') {
            ...
            if (type === 'params') {
              const open = paint(colorful('colors', 'yellow', 'open'))
              const close = paint(colorful('colors', 'yellow', 'close'))

              innerText(elem.children, function (text) {
                return text.split(',').map(arg => open + arg + close).join(',')
              })
