...
module.exports = class TokenStream {
  ...
  async loop () {
    if (!this.quiet) {
      process.stdout.write(COLOR_OPEN('cyan')) // <span class="cyan">
    }

    if (this.callback) {
      this.callback({ type: 'open', value: null })
    }
    ...
    while (true) {
      ...
      if (char === '`') {
        ...
        if (backticksOpen === '```') {
          ...
          if (!this.quiet) {
            process.stdout.write(COLOR_CLOSE('cyan')) // </span>
          ...
          if (this.callback) {
            this.callback({ type: 'search-start', value: { filename } })
          }
          ...
          while (true) {
            ...
            if (line.match(/^={4,10}$/i)) {
              ...
              if (search.length && this.callback) {
                this.callback({ type: 'search-output', value: highlight(search.join('\n'), { language }) })
              }

              if (this.callback) {
                this.callback({ type: 'search-end', value: { lines: search.length } })
              }
          ...
          if (this.callback) {
            this.callback({ type: 'replace-start', value: null })
          }
          ...
          while (true) {
            ...
            if (line.match(/^>{3,10} REPLACE$/i)) {
              ...
              if (replace.length && this.callback) {
                this.callback({ type: 'replace-output', value: highlight(replace.join('\n'), { language }) })
              }

              if (this.callback) {
                this.callback({ type: 'replace-end', value: { lines: replace.length } })
              }
      ...
      if (this.callback) {
        this.callback({ type: 'write', value: char })
      }
    }
    ...
    if (this.callback) {
      this.callback({ type: 'close', value: null })
    }
  }
